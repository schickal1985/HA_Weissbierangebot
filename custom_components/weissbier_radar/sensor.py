...
Provides numeric price sensors with offer tracking and dynamic regular pricing fallbacks.
"""
...
async def async_setup_entry(
    ...
    for prod_id in selected_products:
        ...
        # 1. Best Price Sensor for the product
        entities.append(
            WeissbierBestPriceSensor(coordinator, entry, prod_id)
        )

        # 2. Individual Store Deal / Price Sensors for the product
...
class WeissbierBaseSensor(CoordinatorEntity[WeissbierRadarDataUpdateCoordinator], SensorEntity):
    ...
    def __init__(
        ...
        # Group all sensors under a device per product
        self._attr_device_info = {
            "identifiers": {(DOMAIN, f"{entry.entry_id}_{prod_id}")},
            "name": f"Weissbier Radar - {self.prod_info['name']}",
            "manufacturer": "Weissbier Radar",
            "model": "Kasten (20 x 0,5l)",
            "sw_version": "1.4.0",
        }

    @property
    def product_data(self) -> dict[str, Any]:
        """Return data for this product from coordinator."""
        if not self.coordinator.data:
            return {}
        return self.coordinator.data.get(self.prod_id, {})


class WeissbierBestPriceSensor(WeissbierBaseSensor):
    """Sensor showing the absolute best price found across all stores."""
    ...
    def native_value(self) -> float | None:
        ...
        return float(data.get("normalpreis", DEFAULT_FALLBACK_PRICE))
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        normalpreis = float(data.get("normalpreis", DEFAULT_FALLBACK_PRICE))
...
class WeissbierStoreSensor(WeissbierBaseSensor):
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        normalpreis = float(store_data.get("normalpreis", data.get("normalpreis", DEFAULT_FALLBACK_PRICE)))
        return {
